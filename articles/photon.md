...
`$stop()`. You do not actually *need* to run it manually, because it is
(implicitly) executed on two occasions: 1. on garbage collection and 2.
when the R session ends and external processes are killed.
...
That is a speed increase by a factor of almost 60 (and possibly more on
faster machines)!
