...
#> ℹ A photon executable already exists in the given path. Download will be skipped.
...
#> Error in start_photon(path = self$path, version = private$version, min_ram = min_ram, : Exception in thread "main"
```
...
#> 1 "geocode(\"Apai\", limit = 1)"   17.4ms   19.9ms      50.0    96.5KB     2.08
```
...
#>   expression                          min   median `itr/sec` mem_alloc `gc/sec`
#>   <bch:expr>                     <bch:tm> <bch:tm>     <dbl> <bch:byt>    <dbl>
#> 1 "geocode(\"Apai\", limit = 1)"    253ms    1.04s      1.01     107KB   0.0420
```
